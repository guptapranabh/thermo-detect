...
def process_image(image_path, conf_threshold, iou_threshold):
    """Processes an image file for annotation."""
...
def process_video(video_path, conf_threshold, iou_threshold):
    """Processes a video file for annotation and applies color inversion."""
    ...
    while True:
        ...
        if annotated_frame is not None:
            # Apply color inversion to annotated frames only
...
def predict(input_file, conf_threshold, iou_threshold):
    ...
    if isinstance(input_file, bytes):  # Handle file uploads
        ...
        if image is not None:
            ...
            return process_image(temp_image_path, conf_threshold, iou_threshold)
        else:
